...
| 15 | Order lifecycle state machine | `StatusType` union (`pendingConfirmation` → `restaurantPreparingOrder` → `riderReachingRestaurant` → `riderPickedOrder` → `delivered`) shared between backend and iOS. Non-terminal transitions broadcast as a new `event: status` SSE frame (`TopicChannel.updateStatus`); `delivered` stays terminal, routed through Phase 14's `markAsCompleted()` rather than becoming a plain status frame. `POST /location/complete` renamed to `POST /location/status`, taking the target status in the body. iOS SSE parsing generalizes from a single recognized event (`completed`) to a `StreamEvent` enum (`.location` / `.statusChanged`) so more named events can be added later the same way. Customer map gains a restaurant marker (with a name label) and a decorative curved line to home, shown before a rider exists; the rider "pops in" at `riderReachingRestaurant` and routes to the restaurant first, then to home once `riderPickedOrder` fires, at which point the curved line disappears and the camera narrows from all three markers to rider+home. Rider emulator gets a sequential status-button bar (arrows between steps, disabled-once-passed styling) driving the new endpoint | Multiplexing more than one named SSE event type over a single long-lived connection; deriving client-side routing/camera/UI state from a server-driven status enum instead of only from raw location pings |
...
- **2026-08-30, before what's now Phase 15:** Owner requested the order
  lifecycle state machine (`StatusType`) as "Phase 16," but the next
  unstarted slot in the table above was 15 — Phase 15 had never been used.
  Logged as Phase 15 throughout the tracker/logs; flagging here in case
  the owner meant to reserve 15 for something else not yet described.
